...
import { isB2BOrderRevenueRecognized, isStoreOrderRevenueRecognized } from "./revenue-recognition";
import type {
  AgingBucket,
  DashboardQuery,
  DashboardRange,
 ...
 *   (delivered) AND fully paid. The actual predicate lives in
 *   modules/analytics/revenue-recognition.ts (isB2BOrderRevenueRecognized /
 *   isStoreOrderRevenueRecognized) — the ONE authoritative definition
 *   used here, by the store-orders stats endpoint, and by anywhere else
 *   in the app that needs to know whether a transaction is recognized
 *   revenue, so the numbers can never drift apart between screens again.
 *   An accepted quotation is not revenue. An issued invoice is not
 *   revenue. Even a paid invoice for goods that haven't shipped is not,
 *   on its own, treated as recognized here — this is a delivered-and-paid
 *   policy, which is deliberately conservative. A future finance
 *   configuration could swap this for accrual-on-invoice or another
 *   policy; revenue-recognition.ts is the only place that would need to
 *   change.
...
export async function buildSalesDashboard(
  ...
  for (const o of orders) {
    const inv = o.invoiceId ? invoiceById.get(String(o.invoiceId)) : undefined;
    if (!isB2BOrderRevenueRecognized(o, inv)) continue;
    const recognizedAt = invoicePaidDate(inv as IInvoice);
    if (inRange(recognizedAt)) b2bRevenueRecognized.push({ order: o, invoice: inv as IInvoice, recognizedAt });
  }
  const b2bRevenueRecognizedValue = b2bRevenueRecognized.reduce(
    (sum, { invoice }) => sum + total(invoice.items),
    0,
  );
  const storeRevenueRecognized = storeOrdersInRange.filter(isStoreOrderRevenueRecognized);
