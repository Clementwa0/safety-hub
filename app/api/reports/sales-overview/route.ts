...
import { buildSalesOverviewReport } from "@/modules/analytics/reports";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const report = await buildSalesOverviewReport({ range, start, end });

    return apiSuccess(report, "Sales overview loaded");
  } catch (error) {
    return apiError(
