...
import { buildCustomerInsightsReport } from "@/modules/analytics/reports";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const report = await buildCustomerInsightsReport({ range, start, end });

    return apiSuccess(report, "Customer insights report loaded");
  } catch (error) {
    return apiError(
      error instanceof Error ? error.message : "Failed to load customer insights report",
      [],
      500,
    );
  }
