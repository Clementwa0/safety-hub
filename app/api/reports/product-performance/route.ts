...
import { buildProductPerformanceReport } from "@/modules/analytics/reports";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const report = await buildProductPerformanceReport({ range, start, end });

    return apiSuccess(report, "Product performance report loaded");
  } catch (error) {
    return apiError(
      error instanceof Error ? error.message : "Failed to load product performance report",
      [],
      500,
    );
  }
